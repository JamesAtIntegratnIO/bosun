...
- **Every doc and code comment in the repository is edited for the
  documentation voice.** No em dashes, no emphasis capitals, no filler adverbs,
  no story framing, and an aphorism used once rather than on eight pages. This
  is prose only: `git diff` on `*.go` touches comment lines and nothing else,
  and the report strings the agent parses back are unchanged.

  `adr/` and the changelogs are left alone, as
  [the voice rules](.claude/skills/docs-voice/SKILL.md) say they should be.
