...
rather than somebody's impression of a comment. Nothing is computed: every
answer comes from the snapshot the last sweep already holds, so a request
reaches no git host, no cluster and no model. Nothing mutates,
