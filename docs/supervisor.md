...
`gate_verdict`, `triage_status` and `handoff_queue`. Like the page, the surface
is read-only and reveals operational metadata and no credential; unlike the
page it is off by default and refuses to start without a bearer token, because
it is built to be reached from outside the cluster.
