...
- `inventory`: your fleet, meaning Application names, the namespaces those
  objects live in, and the cluster names they land on. Every Application the
  install's ArgoCD account can list, and not only those of the gated
  repository, so weigh this entry if that account is broader than the team
  holding the token.
