...
[bosun.integratn.io](https://bosun.integratn.io) — Astro + Starlight, deployed
to GitHub Pages by [`.github/workflows/pages.yaml`](../.github/workflows/pages.yaml).
...
`npm run dev` syncs once at startup. A change to a doc **outside** `site/` will
not hot-reload — re-run the command.
...
link against the pages that actually got built, and every rewritten GitHub link
...
**A page that only makes sense on a website** — a landing page, a quickstart
that stitches several docs together — goes in `src/authored/` as `.md` (or
`.mdx` if it needs a component) and gets a sidebar line.
