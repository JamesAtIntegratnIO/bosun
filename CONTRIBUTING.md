...
| The reason the agent's model gave for a handoff, held by `agent`, released by `gateservice`'s sweep, and published by `mcp` | Not yet, and neither half of it fails loudly. Nothing releases a reason unless the composition root wires the sweep's listing to the agent, and an unwired process keeps one model sentence per escalation until the pod restarts; nothing publishes one unless the composition root copies it onto the triage snapshot, and a queue that carries no reason looks exactly like one where bosun stopped for reasons of its own | `mcp_handoff_test.go`: a real `agent.Triage` escalated through the real workflow, its reason driven through the real handler; a real `gateservice` sweep for the release, both when it lists and when it cannot; and the wiring itself read from `main.go`'s syntax tree, so deleting the one line that joins them fails rather than passing. `gateservice/gateservice_test.go` holds the sweep's own half: a listing published when it listed, including the empty one, and none at all when it could not |
...
| What each Application renders from, copied from the gate's rendered rows into `gateservice`'s snapshot and again into `mcp`'s own shapes | Not yet. Two failures share this crossing: a field either copy forgets is chart detail missing from every `inventory` answer, and a values file either copy carries is content on a surface published outside the cluster, arriving one small field at a time | `gateservice/expansion_test.go`: the rendered row and the snapshot walked field for field off `gate.Row`, with every omission named and its reason, so an unexplained third one fails and a values field added back fails twice; `mcp_fleet_test.go` walks the second copy the same way, holds the two spellings of a source type against each other because `mcp` may not import `gate`, and names the values fields where both sides are visible at once |
