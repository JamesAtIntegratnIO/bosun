...
| Every git subprocess redacts what git wrote to stderr | Yes, and the first rule to catch it was itself too narrow: it qualified a function only if it called `pushAuthEnv`, which left the fetch, the ladder and three clones quoting git verbatim into a published report | `gitstderr_test.go`, deriving from the mechanism and not the call sites — a function that starts `git` and reads the buffer it gave that command, across every package rather than `gitprovider` alone |
